...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.buttonOpen.grid(row=0, column=0, padx=10, pady=10, sticky="nesw")

        #--- button to delete image ---
        self.buttonPlot = ctk.CTkButton(self, text="Plot Data", command=self.printData)
        self.buttonPlot.grid(row=2, column=0, padx=10, pady=(0, 10), sticky="nesw")

        #--- button to detect colors ---
        self.buttonDetect = ctk.CTkButton(self, text="Detect QR", command=self.detect_colors)
        self.buttonDetect.grid(row=1, column=0, padx=10, pady=(0, 10), sticky="nesw")

        #==== Frames ====
        #--- frame to display image ---
        self.imageFrame = ctk.CTkFrame(self, fg_color="transparent")
        self.imageFrame.grid(row=0, rowspan=4, column=1, padx=10, pady=10, sticky="nsew")

        #--- frame to display info ---
        self.infoFrame = ctk.CTkFrame(self)
        self.infoFrame.grid(row=3, column=0, padx=10, pady=10, sticky="nesw")
        
        #--- label to display info ---
        self.labelData = ctk.CTkLabel(self.infoFrame, text="Disclaimer:\n Detecting QR codes \nmight take a while", font=("Arial", 15), width=100)
        self.labelData.place(rely=0.1, relx=0.5, anchor="center")
    ...
    def detect_colors(self):
        ...
        #--- display info ---
        self.labelData = ctk.CTkLabel(self.infoFrame, text=f"Text read: {self.dataDisplay}", font=("Arial", 15), width=100)
        self.labelData.place(rely=0.3, relx=0.5, anchor="center")
